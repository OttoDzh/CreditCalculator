...
class DepositViewController: UIViewController , UITextFieldDelegate {
    ...
    @objc func getSum(sum:Double,percentage:Double,days:Double) -> Double {
        ...
        } else if Double(maiView.sum.text!)! > 0 && Double(maiView.percentage.text!)! > 0 && Double(maiView.days.text!)! > 0 { maiView.result.text = "Your amount including accrued % \(totalSum)"
        } else {
            let alertController = UIAlertController(title: "Error!", message: "Enter positive number", preferredStyle: .alert)
            let action = UIAlertAction(title: "Ок", style: .default) {(action) in
            }
